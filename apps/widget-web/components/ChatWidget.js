...
export default function ChatWidget() {
    ...
    const handleSendMessage = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const langResult = yield detectLanguage(inputText);
    ...
    const getBotResponse = (text, lang) => {
        switch (lang) {
            case 'fr':
    ...
    return (<>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {messages.length === 0 ? (<div className="flex items-center justify-center h-full text-gray-500">
          ...
          </div>) : (messages.map(message => (<div key={message.id} className={`mb-4 ${message.sender === 'user' ? 'text-right' : 'text-left'}`}>
              <div className={`inline-block px-4 py-2 rounded-lg ${message.sender === 'user'
                ? 'bg-blue-500 text-white'
                : 'bg-gray-200 text-gray-800'}`} dir={message.sender === 'bot' && (currentLanguage === 'ar' || currentLanguage === 'ar-ma') ? 'rtl' : 'ltr'}>
                {message.text}
              </div>
            </div>)))}
      ...
      <div className="border-t p-4 flex">
        <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder={getPlaceholderText()} className="flex-1 border rounded-l-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" dir={currentLanguage === 'ar' || currentLanguage === 'ar-ma' ? 'rtl' : 'ltr'} disabled={isProcessing}/>
        <button onClick={handleSendMessage} disabled={!inputText.trim() || isProcessing} className="bg-blue-600 text-white px-4 py-2 rounded-r-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-blue-300">
          {getButtonText()}
        </button>
      </div>
    </>);
