...
export default function RealtimeConversationQueue() {
    ...
    const pendingConversations = conversations.filter(conv => conv.status === 'open' || conv.status === 'escalated');
    ...
    return (<div className="bg-white rounded-lg shadow-md p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold" dir={currentLanguage === 'ar' || currentLanguage === 'ar-ma' ? 'rtl' : 'ltr'}>
          ...
          {pendingConversations.length > 0 && (<span className="ml-2 bg-red-100 text-red-800 text-xs font-medium px-2.5 py-0.5 rounded-full">
      ...
      {pendingConversations.length === 0 ? (<div className="text-center py-8 text-gray-500" dir={currentLanguage === 'ar' || currentLanguage === 'ar-ma' ? 'rtl' : 'ltr'}>
          {getText('Aucune conversation en attente', 'لا توجد محادثات في الانتظار', 'ما كاين حتى محادثة فالانتظار')}
        </div>) : (<div className="space-y-3">
          {pendingConversations.map(conversation => (<div key={conversation.id} className={`border rounded-lg p-3 flex justify-between items-center ${conversation.status === 'escalated' ? 'border-purple-500 bg-purple-50' :
                    conversation.slaStatus === 'critical' ? 'border-red-500 bg-red-50' :
                        conversation.slaStatus === 'warning' ? 'border-yellow-500 bg-yellow-50' :
                            'border-gray-200'} ${conversation.source === 'realtime' ? 'animate-pulse' : ''}`} dir={currentLanguage === 'ar' || currentLanguage === 'ar-ma' ? 'rtl' : 'ltr'}>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-medium">{conversation.customer.name}</span>
                  <LanguageIndicator size="sm" showSource={false}/>
                  <span className={`text-xs ${conversation.status === 'escalated' ? 'bg-purple-100 text-purple-800' : 'text-gray-500'} px-2 py-0.5 rounded-full`}>
...
//# sourceMappingURL=RealtimeConversationQueue.js.map