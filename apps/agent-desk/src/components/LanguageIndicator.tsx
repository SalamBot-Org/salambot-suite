...
export default function LanguageIndicator({ size = 'md', showSource = true }: LanguageIndicatorProps) {
  ...
  const getSourceLabel = () => {
    if (!lastDetection || !showSource) return '';
  ...
  return (
    <div className={`flex items-center border rounded-full ${sizeClasses[size]} ${langColors[currentLanguage]}`}>
      ...
      {getConversationCount() > 0 && (
        <span className="inline-flex items-center justify-center w-5 h-5 text-xs font-semibold rounded-full bg-white">
