 ...
 */
export async function detectLanguageCloud(text: string): Promise<LangResult> {
  ...
  try {
    ...
    const cachedResult = await langDetectCache.get(cacheKey);
    ...
    if (cachedResult) {
      ...
      return {
        ...cachedResult,
        source: 'cloud' // Assurer que la source est bien 'cloud'
      };
    }
    ...
    await langDetectCache.set(cacheKey, geminiResult);
    return geminiResult;
  } catch (error) {
