 ...
 */
export async function updateRedisConfig(redisConfig: RedisConfig): Promise<void> {
 ...
 */
export async function getRuntimeConfigForLogs(): Promise<Record<string, unknown>> {
  try {
    const config = await getRuntimeConfig();
    ...
    return {
      redis: {
        ...
        url: config.redis.url ? maskSensitiveValue(config.redis.url) : 'not-set',
